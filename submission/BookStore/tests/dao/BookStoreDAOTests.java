...
public class BookStoreDAOTests {
	...
	void reviewDAONumberQueryProducesCorrectQuery() {
		...
		String query =reviewDAO.newQueryRequest()
				...
				.withResultLimit(73)
				.getQueryString();
		String expectedResult="SELECT RATING,BOOK,CREATED_AT_EPOCH,CUSTOMER,TITLE FROM REVIEW  WHERE RATING >= 3 AND RATING <= 5 AND CREATED_AT_EPOCH >= 10000 AND CREATED_AT_EPOCH <= 9999999 ORDER BY CREATED_AT_EPOCH ASC OFFSET 146 ROWS  FETCH NEXT 73 ROWS ONLY";
		assertEquals(query, expectedResult,"BookDAO number query did not generate the correct query string");
	}
	...
	void bookDAOReferenceEmptyResultIncludeHasCorrectQuery() {
		...
		String query =bookDAO.newQueryRequest()
				...
				.queryCustomers()
				.getQueryString();
		String expectedResult="SELECT BOOK.*,REVIEW.*,CUSTOMER.* FROM BOOK,REVIEW,CUSTOMER WHERE BOOK.ID=REVIEW.BOOK AND CUSTOMER.ID=REVIEW.CUSTOMER FETCH FIRST 20 ROWS ONLY";
	...
	void bookDAOKeyQuery() {
		...
		String query =bookDAO.newQueryRequest()
				...
				.getQueryString();
		String expectedResult="SELECT BOOK.* FROM BOOK  WHERE ID='FAKEID' AND AUTHOR like '%asdf%' FETCH FIRST 20 ROWS ONLY";
		assertEquals(query, expectedResult,"BookDAO did not generate the correct query string");
	}
	...
	void visitorDAOKeyQuery() {
		...
		String query =visitorDAO.newQueryRequest()
				...
				.numberBetween("22", "33")
				.getQueryString();
		String expectedResult="SELECT VISITOR.* FROM VISITOR  WHERE ID='FAKEID' AND CREATED_AT_EPOCH >= 22 AND CREATED_AT_EPOCH <= 33 FETCH FIRST 20 ROWS ONLY";
		assertEquals(query, expectedResult,"BookDAO did not generate the correct query string");
	}
	...
	void reviewDAOKeyQuery() {
		...
		String query =reviewDAO.newQueryRequest()
				...
				.getQueryString();
		String expectedResult="SELECT REVIEW.* FROM REVIEW  WHERE CUSTOMER='fakecust' AND BOOK='FAKEBOOKID' AND CUSTOMER='FAKEID' AND CREATED_AT_EPOCH >= 22 AND CREATED_AT_EPOCH <= 33 AND CUSTOMER='fakecustrebv' AND BOOK='FAKEBOOKIDREB' FETCH FIRST 20 ROWS ONLY";
		assertEquals(query, expectedResult,"BookDAO did not generate the correct query string");
	}
	...
	void customerDAOKeyQuery() {
		...
		String query =customerDAO.newQueryRequest()
				...
				.getQueryString();
		String expectedResult="SELECT CUSTOMER.* FROM CUSTOMER  WHERE ID='fakecustrebv' AND CREATED_AT_EPOCH >= 444 AND CREATED_AT_EPOCH <= 5555 FETCH FIRST 20 ROWS ONLY";
		assertEquals(query, expectedResult,"BookDAO did not generate the correct query string");
	}
	@Test
	void cartDAOKeyQuery() {
		...
		String query =cartDAO.newQueryRequest()
				...
				.isCustomer(new Customer.Builder().withId(new Id("fakecustrebv")).build())
				.getQueryString();
		String expectedResult="SELECT CART.* FROM CART  WHERE ID='fffakeCart' AND USER_TYPE='CUSTOMER' AND BOOK='FAKEBOOKIDREB' AND ID='fakecustrebv' FETCH FIRST 20 ROWS ONLY";
		assertEquals(query, expectedResult,"BookDAO did not generate the correct query string");
	}
	...
	void purchaseOrderDAOKeyQuery() {
		...
		String query =purchaseOrderDAO.newQueryRequest()
				...
				.isShipped()
				.getQueryString();
		String expectedResult="SELECT PURCHASE_ORDER.* FROM PURCHASE_ORDER  WHERE (ID='' AND CREATED_AT_EPOCH=0 AND ID='fakecustrebv' AND ID='FAKEBOOKIDREB' AND STATUS='DELIVERED' AND STATUS='DENIED' AND STATUS='SHIPPED') AND (STATUS='ORDERED' OR STATUS='PROCESSED') FETCH FIRST 20 ROWS ONLY";
		assertEquals(query, expectedResult,"BookDAO did not generate the correct query string");
	}
